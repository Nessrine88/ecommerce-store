import { pgTable, uniqueIndex, uuid, text, integer, numeric, boolean, timestamp, json, foreignKey, primaryKey } from "drizzle-orm/pg-core"
...
export const user = pgTable("User", {
	...
	uniqueIndex("user_email_idx").using("btree", table.email.asc().nullsLast().op("text_ops")),
]);
...
export const accounts = pgTable("accounts", {
	...
	uniqueIndex("account_provider_providerAccountId_idx").using("btree", table.provider.asc().nullsLast().op("text_ops"), table.providerAccountId.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [user.id],
...
export const orderItem = pgTable("OrderItem", {
	...
	primaryKey({ columns: [table.orderId, table.productId], name: "OrderItem_orderId_productId_pk"}),
]);
